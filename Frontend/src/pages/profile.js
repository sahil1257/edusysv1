...
export async function renderProfilePage() {
  ...
  document.getElementById('profile-image-upload').addEventListener('change', async function(event) {
    ...
    if (file) {
        ...
        // Use the new helper function to upload and get the URL
        const imageUrl = await uploadImageFile(file);
        
        // If the upload was successful, update the UI and database
        if (imageUrl) {
            ...
            else { id = currentUser.id; await apiService.update('users', id, { profileImage: imageUrl }); await store.refresh('users'); }
