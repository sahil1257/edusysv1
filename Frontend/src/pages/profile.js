...
export async function renderProfilePage() {
    ...
    document.getElementById('profile-image-upload').addEventListener('change', async function(event) {
        ...
        // Step 1: Provide immediate UI feedback
        const preview = document.getElementById('profile-img-preview');
        const originalSrc = preview.src;
        preview.src = generateInitialsAvatar('...'); // A simple loading indicator
        showToast('Uploading and processing image...', 'info');

        // Step 2: Prepare the "envelope" (FormData)
        const formData = new FormData();
        formData.append('profileImage', file); // Key MUST match backend: upload.single('profileImage')
        formData.append('userId', currentUser.id); // Send user ID for a unique filename

        try {
            // Step 3: Send the file to the server "studio"
            const uploadResult = await apiService.uploadProfileImage(formData);

            // Step 4: Handle the server's response
            if (uploadResult && uploadResult.success) {
                const imageUrl = uploadResult.imageUrl; // The URL, e.g., "/uploads/profile-xyz.jpeg"

                // Update the database record for BOTH the specific profile AND the main user account
                let collection, id;
                ...
                else if (currentUser.role === 'Staff') { collection = 'staffs'; id = currentUser.staffId; }
                else { id = currentUser.id; } // For Admin, Accountant, etc.

                if (collection) {
                    await apiService.update(collection, id, { profileImage: imageUrl });
                }
                await apiService.update('users', currentUser.id, { profileImage: imageUrl });

                // Step 5: Update the UI and local state with the new URL
                preview.src = imageUrl;
                ui.headerUserAvatar.src = imageUrl;
                currentUser.profileImage = imageUrl;
                sessionStorage.setItem('sms_user_pro', JSON.stringify(currentUser));
                
                // Refresh local data stores for consistency
                if(collection) await store.refresh(collection);
                await store.refresh('users');

                showToast('Profile image updated successfully!', 'success');
            } else {
                // If the upload failed, revert the UI and show the error from the server
                preview.src = originalSrc;
                showToast(uploadResult?.message || 'Image upload failed.', 'error');
            }
        } catch (error) {
            // Handle any critical network or unexpected errors
            console.error("The image upload process failed:", error);
            preview.src = originalSrc;
            showToast('A critical error occurred during upload.', 'error');
        }
    });
